...
// bounded wait for a non-blocking connect() to complete, so an
// unreachable/firewalled host can't stall the single-threaded event loop
#define MODBUSMQ_TCP_CONNECT_TIMEOUT_MS 3000
...
modbusmq_tcp_write(modbusmq_context_t *context, int fd, modbusmq_frame_t *frame)
{
    ...
    // MSG_NOSIGNAL: writing to a peer that already closed its end of the
    // connection should fail with EPIPE, not raise SIGPIPE and kill the
    // whole process (the default for a plain write() on a socket).
    rc = send(fd, frame->buf + frame->xmit, frame->length - frame->xmit, MSG_NOSIGNAL);
...
modbusmq_tcp_frame_check(modbusmq_context_t *context, modbusmq_frame_t *frame)
{
    ...
    else // reader
    {
        if (frame->xmit >= 6)
        {
            ...
            if (msg_length > 6 && msg_length <= MODBUSMQ_FRAME_MAX)
            {
...
modbusmq_tcp_read(modbusmq_context_t *context, int fd, modbusmq_frame_t *frame)
{
    ...
    {
        modbusmq_logf(LOG_ERROR, "modbusmq_tcp_read: invalid frame length=%d xmit=%d\n", frame->length, frame->xmit);
        errno = EINVAL;
        return -1;
    }
...
modbusmq_tcp_connect(modbusmq_context_t *context)
{
    ...
    for (addr_rp = addr_result; addr_rp; addr_rp = addr_rp->ai_next)
    {
        ...
        // Set non-blocking before connect() so an unreachable/firewalled
        // host can't stall the whole single-threaded event loop for the
        // OS's default TCP connect timeout (can be tens of seconds).
        int flags = fcntl(sd, F_GETFL);
        if (flags < 0 || fcntl(sd, F_SETFL, flags | O_NONBLOCK | FD_CLOEXEC) < 0)
        {
            modbusmq_logf(LOG_ERROR, "Unable to set socket flags: err=%s\n", strerror(errno));
            close(sd);
            sd = -1;
            continue;
        }
        ...
        else if (errno == EINPROGRESS)
        {
            ...
            rc = poll(&pfd, 1, MODBUSMQ_TCP_CONNECT_TIMEOUT_MS);
            if (rc > 0 && (pfd.revents & POLLOUT))
            {
                int so_error = 0;
                socklen_t so_len = sizeof(so_error);
                if (getsockopt(sd, SOL_SOCKET, SO_ERROR, &so_error, &so_len) == 0 && so_error == 0)
                {
                    rc = 0;
                    break; /* connected */
                }
                modbusmq_logf(LOG_ERROR, "Connect failed: err=%s\n", strerror(so_error));
            }
            else
            {
                modbusmq_logf(LOG_ERROR, "Connect to %s:%s timed out after %d ms\n", tcp->hostname, port, MODBUSMQ_TCP_CONNECT_TIMEOUT_MS);
            }
        }
        else
        {
        ...
        // this candidate address failed, try the next one instead of
        // giving up on the whole hostname immediately
        close(sd);
        sd = -1;
        rc = -1;
    }
    ...
    if (sd < 0)
    {
        modbusmq_logf(LOG_ERROR, "Unable to connect to %s:%s\n", tcp->hostname, port);
        return -1;
    }
