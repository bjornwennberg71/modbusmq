...
static const char *program_version = MODBUSMQ_VERSION_STRING;
...
main(int argc, char **argv)
{
    ...
    while(1)
    {
        ...
        if (FD_ISSET(GI.sd, &read_set))
        {
            ...
            if (new_fd < 0)
            {
                modbusmq_logf(LOG_ERROR, "accept failed: err=%s\n", strerror(errno));
            }
            else
            {
                ...
                if (!accepted)
                {
                    modbusmq_logf(LOG_ERROR, "No free connection slot (max %d), rejecting client\n", MODBUSMQ_CONNECTION_MAX);
                    close(new_fd);
                }
            }
        }
        ...
        for(int i = 0; i < MODBUSMQ_CONNECTION_MAX; ++i)
        {
            ...
            if (FD_ISSET(connection->fd, &write_set))
            {
                ...
                if (modbusmq_frame_incomplete(context, &connection->msg.frame[1]))
                {
                    ...
                    {
                        context->err++;
                        modbusmq_logf(LOG_ERROR, "Unable to write to socket: rc = %d, closing this connection\n", rc);
                        close(connection->fd);
                        connection->fd = 0;
                        continue;
                    }
